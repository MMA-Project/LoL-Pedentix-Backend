...
describe('game.service - full test coverage', () => {
  ...
  describe('startDailyGame', () => {
    ...
    it('should throw if createDailyGame fails', async () => {
      ...
      await expect(gameService.startDailyGame()).rejects.toThrow('Failed to create daily game');
    });
  });

  describe('getPedantixGame', () => {
    it('should throw on invalid id', async () => {
      await expect(gameService.getPedantixGame('invalid')).rejects.toThrow(BadRequestError);
    });

    it('should throw if game not found', async () => {
      (repo.getDailyGame as jest.Mock).mockResolvedValue(null);
      await expect(gameService.getPedantixGame(new ObjectId().toString())).rejects.toThrow(NotFoundError);
    });

    it('should return game', async () => {
      (repo.getDailyGame as jest.Mock).mockResolvedValue({ id: '1', name: 'Ahri' });
      const result = await gameService.getPedantixGame(new ObjectId().toString());
  ...
  describe('getAllChampionsNames', () => {
    it('should return all champion names', async () => {
      (repo.getAllChampions as jest.Mock).mockResolvedValue([{ name: 'Ahri' }, { name: 'Zed' }]);
  ...
  describe('makeGuess', () => {
    ...
    it('should throw BadRequestError for invalid id', async () => {
      await expect(gameService.makeGuess('bad_id', 'word')).rejects.toThrow(BadRequestError);
    });

    it('should throw NotFoundError if game not found', async () => {
      (repo.getDailyGame as jest.Mock).mockResolvedValue(null);
      await expect(gameService.makeGuess(new ObjectId().toString(), 'Lux')).rejects.toThrow(NotFoundError);
    });

    it('should throw NotModifiedError if already guessed', async () => {
      (repo.getDailyGame as jest.Mock).mockResolvedValue({ ...baseGame, guessed: true });
      await expect(gameService.makeGuess(new ObjectId().toString(), 'Ahri')).rejects.toThrow(NotModifiedError);
    });
    ...
    it('should win the game if word is correct', async () => {
      ...
      const result = await gameService.makeGuess(new ObjectId().toString(), 'Ahri');
    ...
    it('should enrich game if word is incorrect and new', async () => {
      ...
      const result = await gameService.makeGuess(new ObjectId().toString(), 'Lux');
...
describe('syncGame', () => {
    ...
    it('should throw BadRequestError on invalid IDs', async () => {
      await expect(gameService.syncGame('bad', 'bad')).rejects.toThrow(BadRequestError);
    });

    it('should throw NotFoundError if existingGame not found', async () => {
      (repo.getDailyGame as jest.Mock).mockImplementationOnce(() => null);
      await expect(gameService.syncGame(new ObjectId().toString(), new ObjectId().toString())).rejects.toThrow(NotFoundError);
    });

    it('should throw NotFoundError if incomingGame not found', async () => {
      const id = new ObjectId().toString();
      (repo.getDailyGame as jest.Mock).mockResolvedValueOnce(baseGame).mockResolvedValueOnce(null);
    ...
    it('should throw NotModifiedError if existing game already guessed', async () => {
      ...
      (repo.getDailyGame as jest.Mock).mockResolvedValueOnce(guessedGame).mockResolvedValueOnce(baseGame);
      await expect(gameService.syncGame(new ObjectId().toString(), new ObjectId().toString())).rejects.toThrow(NotModifiedError);
    });

    it('should throw BadRequestError if seeds mismatch', async () => {
      const game1 = { ...baseGame };
      const game2 = { ...baseGame, seed: 999999 };
      (repo.getDailyGame as jest.Mock).mockResolvedValueOnce(game1).mockResolvedValueOnce(game2);
      await expect(gameService.syncGame(new ObjectId().toString(), new ObjectId().toString())).rejects.toThrow(BadRequestError);
    });
    ...
    it('should merge games and mark guessed if incoming guessed', async () => {
      ...
      (repo.getDailyGame as jest.Mock).mockResolvedValueOnce(existingGame).mockResolvedValueOnce(incomingGame);
      (repo.incFindedCountToHistoryRecord as jest.Mock).mockResolvedValue(1);
      (repo.saveDailyGame as jest.Mock).mockResolvedValue(1);

      const result = await gameService.syncGame(new ObjectId().toString(), new ObjectId().toString());
      const g = result as any;
